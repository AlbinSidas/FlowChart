...
async function main() {
    ...
    app.get('/', (req, res, next) => res.json({ apa: 'Hello World!' }));

    // ================ FLOWCHART ====================
    app.post('/flowchart/save', async function (req, res, next) {
        try {
            const databaseOps = await mongoController.flowchartHandler.save(
                req.body,
            );
            res.status(200);
    ...
    app.get('/flowchart/all', async function (req, res, next) {
        try {
    ...
    app.get('/flowchart/view', async function (req, res, next) {
        try {
    ...
    app.post('/flowchart/version/add', async (req, res, next) => {
        try {
            const data = req.body;
            // SCHEMA VALIDATION?
            const databaseOps = await mongoController.flowchartHandler.addVersion(
                data,
            );
            res.status(200);
    ...
    app.get('/flowchart/version/snapshot/:id', async (req, res, next) => {
        try {
            const result = await mongoController.flowchartHandler.getVersionSnpashot(
                req.params.id,
            );
            res.status(200);
    ...
    app.get('/flowchart/:id/:version?', async function (req, res, next) {
        try {
            const databaseOps = await mongoController.flowchartHandler.getOne(
                req.params.id,
                req.params.version,
            );
            res.status(200);
    ...
    // ================= FUNCDEF =====================
    app.post('/funcdef/version/add', async (req, res, next) => {
        try {
            const data = req.body;
            // SCHEMA VALIDATION?
            const databaseOps = await mongoController.funcDefHandler.addVersion(
                data,
            );
            res.status(200);
            res.json(Response('Versioned function definition', databaseOps));
    ...
    app.get('/funcdef/version/snapshot/:id', async (req, res, next) => {
        try {
            const result = await mongoController.funcDefHandler.getVersionSnpashot(
                req.params.id,
            );
            res.status(200);
            res.json(Response('Fetched version numbers', result));
    ...
    app.get('/funcdef/all', async (req, res, next) => {
        try {
    ...
    app.post('/funcdef/save', async (req, res, next) => {
        const data = req.body;
        try {
    ...
    app.get('/funcdef/:id/:version?', async (req, res, next) => {
        try {
            const databaseOps = await mongoController.funcDefHandler.getOne(
                req.params.id,
                req.params.version,
            ); // kan behöva kallas på från någon annanstans om det blir större
            res.json(Response('', databaseOps));
        } catch (err) {
            next(err);
        }
    ...
    app.listen(serverConfig.port, () =>
        console.log(
            `Foran Flowchart server listening on port ${serverConfig.port}!`,
        ),
    );
}
