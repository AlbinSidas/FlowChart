...
class Network {
    ...
    async getAll() {
        const data = await fetch(`${this.baseURL}/all`).then((res) =>
            res.json(),
        );
    ...
    async getById(id) {
        const data = await fetch(`${this.baseURL}/${id}`).then((res) =>
            res.json(),
        );
        return this.transformerInterface.resObject(data);
    }
...
class FlowchartAPI extends Network {
    ...
    async getVerNums(id) {
        const data = await fetch(
            `${this.baseURL}/version/snapshot/${id}`,
        ).then((res) => res.json());
        return this.transformerInterface.findVerNumbers(data, id);
    }
    async getFlowVersion(id, ver) {
        const data = await fetch(`${this.baseURL}/${id}/${ver}`).then((res) =>
            res.json(),
        );
        return this.transformerInterface.resObject(data);
    }
