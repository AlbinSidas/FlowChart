...
class Container extends View {
        ...
        eventEmitter.on('openedFlowchart', (chosenFlowchart) => {
            ...
            looseNodes.forEach((looseNode) => {
                if (looseNode.type == 'flowchart_node') {
                    ...
                    this.objects.push(
                        ConditionalNode.CreateExternal(looseNode),
                    );
        ...
        for (let i = 0; i < this.connectorList.length; i++) {
            if (this.connectorList[i].currNode.id == obj.id) {
                for (let j = 0; j < this.markedObject.length; j++) {
                    if (
                        this.markedObject[j].id ==
                            this.connectorList[i].prevNode.id &&
                        this.markedObject[j].id != obj.id
                    ) {
                ...
                for (let j = 0; j < this.markedObject.length; j++) {
                    if (
                        this.markedObject[j].id ==
                            this.connectorList[i].currNode.id &&
                        this.markedObject[j].id != obj.id
                    ) {
                        this.connectorList[i].glow();
                    }
    ...
    async loadFlow() {
        this.clearFlowchart();
        const loadedObjects = await this.saveClass.loadFlowVer(
            this.flowchartId,
            this.currentFlowchartVer,
        );
    ...
    onClick(e) {
        if (
            ...
            this.markedObject.length != 0
        ) {
            this.removeMarked();
            this.removeMarkedConnector();
        }
        if (
            (e.clientX != this.connectorClick.clientX ||
                e.clientY != this.connectorClick.clientY) &&
            this.markedConnector.length != 0
        ) {
            this.removeMarkedConnector();
        }
    }
    ...
    removeNode() {
        ...
        if (this.markedObject.length != 0) {
            for (let j = this.markedObject.length - 1; j >= 0; j--) {
                this.objects.splice(
                    this.objects.indexOf(this.markedObject[j]),
                    1,
                );
                for (let i = this.connectorList.length - 1; i >= 0; i--) {
                    if (
                        this.connectorList[i].id.includes(
                            this.markedObject[j].id,
                        )
                    ) {
    ...
    decreaseSize() {
        if (window.innerHeight < this.height - this.sizeDelta) {
            for (let i = 0; i < this.objects.length; i++) {
                ...
                if (
                    flowchartNode.getPosY() + flowchartNode.getHeight() >
    ...
    decreaseSizeHorizontal() {
        if (window.innerWidth < this.width - this.sizeDelta) {
            for (let i = 0; i < this.objects.length; i++) {
                const flowchartNode = this.objects[i];
                if (
                    flowchartNode.getPosX() + flowchartNode.getWidth() >
    ...
    async asyncFlowSave() {
        try {
            ...
            await this.saveClass.saveFlowVer(
                this.objects,
                this.flowchartName,
                this.flowchartId,
            );
    ...
    async newFlowSetup(name) {
        ...
        let flowObj = await this.saveClass.saveFlow(
            this.objects,
            this.flowchartName,
        );
    ...
    async uppdateVerNum() {
        ...
        document.getElementById(
            'vercounter',
        ).innerHTML = this.currentFlowchartVer;
    }
